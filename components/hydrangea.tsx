...
export function Hydrangea({ className = "", size = "md", variant = "medium" }: HydrangeaProps) {
  const dimension = sizeMap[size]
  const colors = colorVariants[variant]

  // Generate flower petals in a circular pattern
  ...
  return (
    <div className={`animate-sway ${className}`} style={{ animationDelay: `${Math.random() * 2}s` }}>
      <svg
        width={dimension}
        height={dimension}
      ...
      >
        ...
        <g filter={`url(#glow-${variant})`}>{generatePetals()}</g>
      </svg>
    </div>
  )
}
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="flex items-end justify-center gap-[-20px]">
        <Hydrangea size="md" variant="light" className="-mr-6 z-10" />
        <Hydrangea size="lg" variant="medium" className="z-20 -mb-2" />
        <Hydrangea size="md" variant="dark" className="-ml-6 z-10" />
      </div>
    </div>
  )
}
